...
class Home_RecyclerAdapter01 extends RecyclerView.Adapter<Home_RecyclerAdapter01.ViewHolder> {
    private Context context;
    private HomeBean homeBean;
    private ViewHolder holder;

    public Home_RecyclerAdapter01(Context context, HomeBean homeBean) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder viewHolder, final int i) {
        viewHolder.home_text_price01.setText("¥"+homeBean.getResult().getRxxp().getCommodityList().get(i).getPrice()+"");
        viewHolder.home_text_title01.setText(homeBean.getResult().getRxxp().getCommodityList().get(i).getCommodityName());
        //Glide.with(context).load(rb1Bean.getResult().getPzsh().get(0).getCommodityList().get(i).getMasterPic()).into(holder.view_image);
        Uri uri = Uri.parse(homeBean.getResult().getRxxp().getCommodityList().get(i).getMasterPic());
        viewHolder.home_icon01.setImageURI(uri);
        viewHolder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                EventBus.getDefault().postSticky(homeBean.getResult().getRxxp().getCommodityList().get(i).getCommodityId()+"");
    ...
    public int getItemCount() {
        return homeBean.getResult().getRxxp().getCommodityList().size();
    }
